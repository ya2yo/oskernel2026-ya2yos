...
impl Ext4Inode {
    ...
    pub(super) fn write_at_impl(&self, off: usize, buf: &[u8]) -> SyscallRet {
        ...
        // stable while an in-memory write bypasses the backing filesystem.
        ...
        let (path, current_size) = {
            ...
            // VFS wrapper.  The lwext4 C entry points additionally take the
            // resource-level inode/journal locks they actually require.
