...
const SERIAL_SCRIPT_PATH: &str = "/tmp/buildstorm-xtask-prebuild-serial.sh\0";
...
const SERIAL_SCRIPT_BODY: &str = r#"#!/bin/sh
mount -t proc proc /proc 2>/dev/null
mount -t sysfs sysfs /sys 2>/dev/null
mount -t devtmpfs devtmpfs /dev 2>/dev/null
export PATH=/root/.cargo/bin:/usr/local/bin:/usr/bin:/bin:/sbin:/usr/sbin
...
cd /work/tgoskits 2>/dev/null || {
    echo "BUILDSTORM_DEBUG_XTASK_PREBUILD_SERIAL fail stage=chdir"
    exit 1
}
echo "----- pre-build tg-xtask serial diagnostic (untimed) -----"
cargo build -p tg-xtask --jobs 1 2>&1
RC=$?
if [ "$RC" -eq 0 ]; then
    echo "BUILDSTORM_DEBUG_XTASK_PREBUILD_SERIAL ok"
    exit 0
fi
echo "BUILDSTORM_DEBUG_XTASK_PREBUILD_SERIAL fail rc=$RC"
exit "$RC"
"#;
...
/// Diagnose the serialized lwext4 workload without changing the default case.
#[allow(dead_code)]
pub fn run_serial() -> i32 {
    common::run_case(
        "xtask-prebuild-serial",
        SERIAL_SCRIPT_PATH,
        SERIAL_SCRIPT_BODY,
    )
}
