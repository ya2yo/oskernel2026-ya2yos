...
4. **Linux ABI 深度兼容**：实现了包括 `robust_list`、`clear_child_tid`、`CLONE_CHILD_CLEARTID`、`copy_file_range`、`userfaultfd` 等在内的复杂 Linux 特性，使得 glibc 等标准库能近乎无修改运行，通过了 libc-test 的大量测试用例；
...
| `spin` | 自旋锁（内核态同步原语） |
| `hashbrown` | 高性能哈希表 |
| `futures-util` | 异步编程工具（用于网卡轮询 Waker 机制） |
