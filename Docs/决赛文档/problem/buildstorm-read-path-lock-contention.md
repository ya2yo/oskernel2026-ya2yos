...
`panic`、`TFAIL`、`TBROK`、`ERROR`、`SIGSEGV` 或 rustc `error:`；最后快照为 `t=1787649ms`，
Cargo 为 `Building 65/446`，没有 `BUILDSTORM_COMPILE`、测试组 END 或 `shutdown!`。因此这不是完整
BuildStorm 验收样本，也不是“成功编译后无输出”；它是在 Cargo 仍有重 crate 正在编译或等待时被截断的中途窗口。

末尾累计统计为 `ext4_fstat_lock=4737`（wait `880.098s`、hold `145.228s`）、实际
`ext4_fstat=4729`、`directory_epoch_cached=3310`；目录 epoch miss 为 `4722`，其中 local `3258`、global
`2653`。`tmp_07` 只到 `539771ms/23--24/446`，而 `tmp_08` 运行时间和 Cargo 阶段均不同，故这些累计值不能作为
严格 A/B。更重要的是，`tmp_08` 没有本轮新增的 `ext4_fstat_directory_parent` 行，说明它尚未装载父目录局部失效
代码；本轮收益必须由下一份样本重新验证。

同时，`tmp_08` 仍显示页缓存容量未触顶（`82368/98304`，capacity bypass `0`），`mmap_prefetch=0`，
四类 `inode_read_source` 继续守恒。因此当前没有证据支持继续扩大文件页缓存准入、全局容量或预读深度。

### 为什么长时间没有编译成功输出

Cargo 进度只在 crate 完成或新 crate 启动时刷新。最后 perf 快照仍持续打印，说明 guest 没有死机。最后约 39 秒
interval 中 `ext4_read_data_lock` 等待约 `65.7s`、`ext4_fstat_lock` 等待约 `13.6s`、`ext4_write_lock`
等待约 `89.3s`，`pipe_read_wait` 约 `108.4s`；这些是并发任务的累计睡眠时间，不能直接相加成 wall-clock，也不能单独
断言某一个锁是唯一根因。

### 本轮实现与下一轮取证计划

本轮在保持全局 lwext4 gate 与锁序不变的前提下，增加父目录局部 stat epoch 失效：普通文件
rename/hard-link/unlink/延迟删除会尝试从 `FsIndex` 找到已缓存父目录并只更新其 epoch；未缓存父目录回退全局
epoch，目录 rename/rmdir 保持全局失效。perf 新增 `ext4_fstat_directory_parent local_updates/global_fallbacks`，
但 `tmp_08` 尚未包含该字段。

下一步先用同入口样本验收上述计数和 fstat 锁变化，而不是直接移动 lwext4 全局 gate：

1. 检查 `ext4_fstat_directory_parent` 的 local/global 比例，并比较 `directory_epoch_cached`、epoch miss、实际
   fstat 及 `ext4_fstat_lock wait/hold`。
2. 若 global fallback 仍占主导，定位父目录未进入 `FsIndex` 的路径；若 fstat 已下降，再对相同 Cargo 检查点做至少两次
   运行，避免把运行时长或并发交错误认为加速。
3. 保留已有 interval delta、namespace/create 子阶段和 pipe wait 统计；只有确认对应 interval 由某一子阶段主导后再改
   lwext4 gate 或 VFS/cache 收尾。

## 2026-07-31：P15.1 父目录局部 stat epoch 失效

### 实现

目录 stat cache 现在同时保存本 inode epoch 和 mount-wide fallback epoch。新增 VFS no-op hook
`mark_directory_stat_changed()`，EXT4 在普通文件 namespace 操作成功后解析路径父目录：已缓存父目录只推进其本地
epoch，未缓存父目录或无法拆分的路径才推进全局 epoch。目录 rename/rmdir 继续使用全局失效，以覆盖目录自身 ctime 和
目标替换等保守边界。

### 验证边界

本轮已通过 RISC-V/LoongArch64 perf 构建、格式检查和 `git diff --check`；`tmp_08.ans` 是修改前的 30 分钟样本，
没有 `ext4_fstat_directory_parent` 字段，不能作为本轮 A/B。下一份样本应重点检查该字段，并以同 Cargo 检查点比较
`actual_ext4_fstat`、`directory_epoch_cached`、epoch miss 拆分和 `ext4_fstat_lock` wait/hold；在完整
`BUILDSTORM_COMPILE ... ok=true`、测试组 END、`shutdown!` 出现前，不报告端到端吞吐提升。
