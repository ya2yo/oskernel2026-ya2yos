...
- bcache hit/miss/loader/wait/error/evict/writeback、block request queue/wait/completion 尚无累计和 interval telemetry。
- inode identity、open-file `f_pos`、目录、allocator 和 journal 仍是串行语义，metadata 并发不在本轮范围。

## 下一步优化方案

1. **P21.2a telemetry。** 增加累计和 interval 的 `hit/miss/loader/wait/load_error/evict/full_dirty/writeback`，
   waiter 时长/最大值，以及 block request submit/wait/complete；先在旧 gate 下验证计数守恒和零额外唤醒。
2. **P21.2b 容量与等待收敛。** index lock 下为 victim 声明 `EVICTING` 并 pin，锁外 drop 或 writeback；shared
   调用遇到 dirty-only 满缓存时返回 `NEED_EXCLUSIVE`，由上层释放共享准入后在 exclusive fallback 中回写并重试。
   waiter 按 LBA hash 分桶并保留状态二次检查，避免全 mount wake-all。
3. **P21.2c oracle。** 覆盖同 LBA 单飞、不同 LBA 并发、dirty/flush/evict 竞争、加载和写回错误、满缓存退让与
   10 万次随机生命周期；检查 `refctr`、tree/list membership、`ref_blocks <= cnt + transient_loaders` 和数据内容，
...
4. **P21.3 只读并发。** 先建立稳定 inode number/generation 对应的 `Ext4InodeState` 和独立 open-file `f_pos`，
   ...
   明显下降。
5. **性能验收。** 固定镜像、冷启动、QEMU `-smp`、Cargo jobs 和 crate checkpoint 做两次 A/B；最终以两次完整
   `BUILDSTORM_COMPILE ... ok=true`、测试组 `END`、`shutdown!` 和无 fsck 差异为准，不用本次 `13/446` 截断样本
   宣称加速或回退比例。
...
```text
make musl-generic ARCH=riscv64 LWEXT4_BUILD_DIR=build_musl-generic-riscv64-p21
make musl-generic ARCH=loongarch64 LWEXT4_BUILD_DIR=build_musl-generic-loongarch64-p21
make build-arch TARGET_ARCH=riscv64
make build-arch TARGET_ARCH=loongarch64
git diff --check
```

RISC-V 8 HART BuildStorm 冒烟通过启动、`BUILDSTORM_TOOLCHAIN ok` 和 `BUILDSTORM_MINIBUILD ok`，随后停在 untimed
`pre-build tg-xtask`，未出现 panic、lwext4 assert、TFAIL 或 TBROK；该样本不是完整 BuildStorm，也没有并发 bcache
telemetry。P21.2 oracle、I/O 错误注入、fsck 和完整 BuildStorm 尚待执行。
